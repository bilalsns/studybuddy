...
# matching modulidagi funksiyalar va interests uchun categories
...
# Foydalanuvchilarning tilini xotirada saqlash (user_id: language_code)
user_language = {}

def language_keyboard():
    # Aiogram 3.x uchun inline_keyboard parametri orqali tugmalarni qatorda joylaymiz
    inline_kb = [[
...
# Til (language) bo'yicha komandalar uchun router
lang_router = Router()

@lang_router.message(Command("language"))
async def select_language(message: types.Message):
    if message.from_user.id not in user_language:
        user_language[message.from_user.id] = "uz"  # Standart til sifatida
...
# Dispatcher va barcha routerlar (main, profile, edit, language)
...
# CONSTANTS (Komandalar va xabarlar)
COMMAND_START = "start"
COMMAND_BAN = "ban"
COMMAND_UNBAN = "unban"
